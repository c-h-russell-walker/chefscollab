...
function hl_twitter_display_archive_page($arg_username = false, $arg_year = false, $arg_month = false, $arg_day = false, $arg_page = false, $arg_search = false) {
	...
	if($arg_username != '-' and $arg_username != '') {
		...
		$temp_user_id = $wpdb->get_var($wpdb->prepare('
	...
	if($arg_search!='') {
		...
		if(!empty($temp_search)) $hl_twitter_params->search = $temp_search;
	}
	...
	$data->users = $wpdb->get_results('
		SELECT screen_name, name, num_tweets, avatar
		FROM ' . HL_TWITTER_DB_PREFIX . 'users
	...
	if(!$distinct_months) {
		$distinct_months = $wpdb->get_results('
			SELECT YEAR(created) AS year, MONTH(created) AS month, COUNT(*) AS num
